...
@section('content')
    <!-- Page Content -->
        <!-- Dynamic Full -->
        <div class="block">
            <form action="{{url('/cards/store/'.$card->id)}}" method="post">
                @csrf
                <div class="block-header block-header-primary">
                    <h3 class="block-title">{{ __('card.update') }}</h3>
                </div>
                <div class="block-content block-content-full">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('expiration') is-invalid @enderror" id="expiration" name="expiration" value="{{ old('expiration', $card->expiration) }}">
                            @error('expiration')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="country_id" class="form-control @error('country_id') is-invalid @enderror" id="country_id">
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ ( $country->id == $card->country_id) ? 'selected' : '' }} >{{ $country->country }}</option>
                                @endforeach
                            </select>
                            @error('country_id')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="state_id" class="form-control @error('state_id') is-invalid @enderror" id="state_id">
                                ...
                                @foreach($states as $state)
                                    <option value="{{ $state->id }}" {{ ( $state->id == $card->state_id) ? 'selected' : '' }} >{{ $state->state }}</option>
                                @endforeach
                            </select>
                            @error('state_id')
                                <span class="invalid-feedback" role="alert">
                    ...
                    <div class="form-group row">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control @error('card_address_postal_code') is-invalid @enderror" id="card_address_postal_code" 
                                name="card_address_postal_code" value="{{ old('card_address_postal_code', $card->card_address_postal_code) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('card_address_city') is-invalid @enderror" id="card_address_city" 
                                name="card_address_city" value="{{ old('card_address_city', $card->card_address_city) }}">
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control @error('card_address') is-invalid @enderror" id="card_address" 
                                name="card_address" value="{{ old('card_address', $card->card_address) }}">
                            @error('card_address')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                ...
                <div class="block-content block-content-full text-right">
                    <a href="{{url('cards/'.$card->access_id)}}" class="btn btn-secondary pull-left">
                      {{__('default.btn_back')}}
                    </a>
